...
def main():
    ...
    if refresh_odds:
        ...
        try:
            with st.spinner("Fetching current NFL market spreads…"):
                outcome = sync_odds(store)
            st.success(f"Saved {outcome['lines']} bookmaker spread lines.")
        except (AgainstAllOddsError, OSError, sqlite3.Error) as error:
    ...
    if view == "Availability":
        ...
        left, middle, right, receiver, skill, edge = st.columns(6)
        ...
        if skill.button("Build RB/TE profiles", width="stretch"):
            try:
                counts = build_rb_te_profiles(store)
                st.success(f"Built {counts['running_backs']} RB and {counts['tight_ends']} TE role-based profiles.")
            except (AgainstAllOddsError, OSError, sqlite3.Error) as error:
                st.error(f"RB/TE profile build failed: {error}")
        if edge.button("Build EDGE profiles", width="stretch"):
            try:
                st.success(f"Built {build_edge_profiles(store)['players']} rolling EDGE disruption profiles.")
            except (AgainstAllOddsError, OSError, sqlite3.Error) as error:
                st.error(f"EDGE profile build failed: {error}")
        report = current_report(store)
        profile = availability.latest_profile()
        wr_profile = availability.latest_wr_profile()
        rb_te_profile = availability.latest_rb_te_profile()
        edge_profile = availability.latest_edge_profile()
        ...
        status.write({"latest_official_snapshot": report.get("retrieved_at"), "report_age_hours": freshness["age_hours"], "latest_qb_profile": profile and profile["generated_at"], "latest_WR_profile": wr_profile and wr_profile["generated_at"], "latest_RB_TE_profile": rb_te_profile and rb_te_profile["generated_at"], "latest_EDGE_profile": edge_profile and edge_profile["generated_at"]})
        ...
        if profile:
            ...
            if rb_te_profile:
                skill_profiles = json.loads(rb_te_profile["profiles"])
                st.subheader("RB and TE role-based inputs")
                st.caption("These replacements come from the same rushing or receiving role in play-by-play data, not a roster depth chart. WR, RB, and TE effects are capped at three points per team together.")
                rb_column, te_column = st.columns(2)
                rb_column.dataframe(pd.DataFrame(skill_profiles["RB"]), hide_index=True, width="stretch")
                te_column.dataframe(pd.DataFrame(skill_profiles["TE"]), hide_index=True, width="stretch")
            if edge_profile:
    ...
    if view == "Games":
        ...
        filter_model, filter_family, filter_season, filter_week = st.columns([1.25, 1.15, .7, .55])
        with filter_model:
            model = st.selectbox("Projection model", list(LABELS), format_func=LABELS.get)
        with filter_family:
            family = st.selectbox("Feature family", ["raw", "rolling-adjusted", "srs"], format_func=lambda value: {"raw": "Raw rolling rates", "rolling-adjusted": "Opponent-normalized", "srs": "Season-long SRS"}[value], disabled=model == "power-rating-v1")
        with filter_season:
            season = st.selectbox("Season", seasons)
        weeks = sorted({g.week for g in games if g.season == season})
        with filter_week:
            week = st.selectbox("Week", ["All", *weeks])
        imported = st.file_uploader("Import reviewed market CSV", type="csv", help="Columns: home_team, away_team, bookmaker, home_spread; optional kickoff and captured_at.")
        if imported is not None and st.button("Save imported market odds"):
            from againstallodds.odds import import_odds
            try:
                outcome = import_odds(store, imported.getvalue())
                st.success(f"Saved {outcome['lines']} bookmaker spread lines from CSV.")
            except (AgainstAllOddsError, OSError, sqlite3.Error) as error:
                st.error(f"Market CSV import failed. {error}")
        ...
        if model != "power-rating-v1":
            ...
            projections.update({r["game_id"]: r for r in predict_models(store, model=model, family=family)})
        display = []
        for game in games:
            if game.season != season or (week != "All" and game.week != week):
                continue
            p = projections.get(game.game_id, {})
            display.append({"Date": game.gameday, "Week": game.week, "Stage": game.game_type, "Away": game.away_team, "Home": game.home_team, "Score · away–home": f"{game.away_score}–{game.home_score}" if game.complete else "Pending", "Projected line": format_line(p), "Market consensus": format_line({**p, "predicted_margin": p.get("market_margin")}) if p.get("market_margin") is not None else "—", "Model edge": "—" if p.get("edge") is None else f"{p['edge']:+.1f}", "Books": p.get("market_books", "—"), "Projection type": p.get("exclusion") or ("Historical simulation" if "actual_margin" in p else "Upcoming estimate" if p else "Not available"), "Neutral": game.neutral_site})
